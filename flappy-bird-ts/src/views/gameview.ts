import { Pipe } from "../model/gamebrain";
...
export class GameView {
    ...
    birdHop() {
        let bird = document.querySelector("#bird") as HTMLDivElement;
        let rect = bird!.getBoundingClientRect();
        bird!.style.transition = "top 0.3s";
        bird!.dataset.top = (Number(bird!.dataset.top) - rect.height * 4).toString();
        bird!.style.top = bird.dataset.top + "px";
    }

    birdFall() {
        let bird = document.querySelector("#bird") as HTMLDivElement;
        bird!.dataset.top = (Number(bird!.dataset.top) + 3).toString();
        bird.style.top = bird.dataset.top + "px";
    }

    drawBoard(gameBoard: Pipe[]) {
...
function createBird() {
    ...
    bird.dataset.top = (window.innerHeight / 2).toString();
...
function createPipe(pipe: Pipe) {
    ...
    elemPipe.dataset.upperHeight = (window.innerHeight / 100 * pipe.upperHeight).toString();
    elemPipe.dataset.lowerHeight = (window.innerHeight / 100 * pipe.lowerHeight).toString();
    elemPipe.dataset.xCoord = (pipe.xCoord).toString();
    ...
    elemPipe.dataset.right = (pipe.xCoord).toString();
